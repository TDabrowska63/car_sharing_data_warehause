...
Select [Measures].[Liczba zgloszen] On Columns,
	Order({([ID Daty Zgloszenia].[Hierarchy].[2019].[February], [Dim Zgloszenie].[Nr Telefonu Zglaszajacego].members),
...
Select [Measures].[Liczba zgloszen] On Columns,
	Order({([ID Daty Zgloszenia].[Year].[2020], [Dim Zgloszenie].[Nr Telefonu Zglaszajacego].members),
...
Select [Measures].[Liczba zgloszen] On Columns,
	{([ID Daty Zgloszenia].[Hierarchy].[2019].[February], [Dim Uzytkownik].[Nr Prawa Jazdy].members)} On Rows
	from [Fakty Wypozyczenia];
